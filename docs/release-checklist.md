...
- The public contract maintains 65 MCP tools, 2 MCP resources, and 2 prompt templates.
- `stage_service_definition` is exposed to convert `.sddraft` to `.sd` before publishing.
- The Add-In maintains 54 internal commands; `ping` is an internal IPC command only.
